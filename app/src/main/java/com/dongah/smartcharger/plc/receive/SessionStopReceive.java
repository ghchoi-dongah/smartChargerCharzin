...
public class SessionStopReceive {

    private static final Logger logger = LoggerFactory.getLogger(SessionStopReceive.class);

    byte[] data;
    int len;
    DataTransformation dataTransformation = new DataTransformation();

    public SessionStopReceive(byte[] data, int len) {
        this.data = data;
        this.len = len;
    }
    ...
    public void doReceive() {
        try {
            ...
            logger.error(" SessionStopReceive error : {}", e.getMessage());
        }
    }
}
